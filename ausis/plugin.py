...
class Plugin(pluginmanager.IPlugin, PluginStorage):
    ...
    def init(self, ausis_api):
        """
        Called before ausis starts;
        Passes ausis_api object for plugins to do initialization.
        ...
        if self.is_callable_plugin():
            if hasattr(
                ...
                getattr(
                    self._backend[0].__class__,
                    "init")):
                self._backend[0].init(ausis_api)
        for plugin in self.get_plugins().values():
            plugin.init(ausis_api)
    ...
    def run(self, ausis, s):
        ...
        sub_command = ausis.find_action(s, self.get_plugins().keys())

        if sub_command == "None":
            # run default
            if self.is_callable_plugin():
                self._backend[0](ausis.get_api(), s)
            else:
                ausis.get_api().say("Sorry, I could not recognise your command. Did you mean:")
                for sub_command in self._sub_plugins.keys():
                    ausis.get_api().say("    * {} {}".format(self.get_name(), sub_command))
        else:
            command = sub_command.split()[0]
            new_s = " ".join(sub_command.split()[1:])
            self.get_plugins(command).run(ausis, new_s)

    def _plugin_run_with_network_error(self, run_func, ausis, s):
        """
        Calls run_func(ausis, s); try-catch ConnectionError
        ...
        try:
            ...
            ausis.get_api().connection_error()