...
def checkWinner(board, ausis, turn):
    if board['7'] == board['8'] == board['9'] != '   ':
        printBoard(board)
        ausis.say("\n--- Game Over ---\n", Fore.GREEN)
        ausis.say(turn + " won!", Fore.GREEN)
        return True
...
def game(ausis, s):
    ...
    while count < 10:
        printBoard(board)
        s = ausis.input(turn + "turn. " + "Choose a position!", Fore.BLUE)
        ...
        if count >= 5:
            if(checkWinner(board, ausis, turn)):
                break
        ...
        if count == 9:
            printBoard(board)
            ausis.say("\n--- Game Over ---\n", Fore.GREEN)
            ausis.say("It's a Draw!", Fore.GREEN)
            break
