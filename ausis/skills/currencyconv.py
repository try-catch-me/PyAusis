...
class Currencyconv():
    ...
    def __call__(self, ausis, s):
        ...
        amount = ausis.input_number('Enter an amount: ')
        from_currency = self.get_currency(ausis, 'Enter from which currency: ', currencies)
        to_currency = self.get_currency(ausis, 'Enter to which currency: ', currencies)

        self.currencyconv(ausis, amount, from_currency, to_currency)

    def currencyconv(self, ausis, amount, fr, to):
        """
    ...
    def get_currency(self, ausis, prompt, currencies):
        """
