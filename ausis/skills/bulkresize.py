...
def spin(ausis, s):
    ...
    ausis.say(
        ...
        Fore.YELLOW)
    ausis.say('Press y for "YES" n for "NO"', Fore.YELLOW)
    rename = ausis.input()
    ausis.say('Enter the path of output directory :', Fore.YELLOW)
    path2 = ausis.input()
    if not dir_exist(path2):
        ausis.say(
            'The path ' + path2 + ' does not exist. Do you want to create it?',
            Fore.YELLOW)
        ausis.say('Print y for "YES" n for "NO"', Fore.YELLOW)
        answer = ausis.input()
    if answer is 'y':
        create_dir(path2)
    else:
        while not dir_exist(path2):
            ausis.say(
                ...
                Fore.YELLOW)
            path2 = ausis.input()
    ausis.say("Enter the target size :", Fore.YELLOW)
    size = ausis.input_number(rtype=int)
    if rename == 'y':
        bulk_resizer(path1, path2, size, [0, 0, 0], True)
    else:
        bulk_resizer(path1, path2, size, [0, 0, 0], False)
    ausis.say("Resizing Compleated!! Thank you for using ausis", Fore.GREEN)
