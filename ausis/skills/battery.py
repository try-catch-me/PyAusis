...
def battery_WIN32(ausis, s):
    ...
    if batt.power_plugged:
        ausis.say("Battery is charging: %s%%" % batt.percent)
    else:
        ausis.say("charge = %s%%, time left = %s" %
                   (batt.percent, secs2hours(batt.secsleft)))


@require(platform=LINUX, native='upower')
@plugin('battery')
def battery_LINUX(ausis, s):
    """
    ...
    ausis.say(get_specific_info(s.lower()))
...
@require(platform=LINUX, native='!upower')
@plugin('battery')
def battery_LINUX_FALLBACK(ausis, s):
    """
    Provides battery status like battery percentage
    and if the battery is charging or not
    """
