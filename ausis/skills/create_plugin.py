...
to ecourage people to experiment with ausis's plugins.
"""
...
and locate the ausis/custom folder through relative pathing.
"""
...
def create_plugin_MAC(ausis, s):

    # ausis asks for the name of the plugin to create if not provided.
    if s == "":
        ausis.say("Please insert the name of your plugin: ", Fore.RED)
        name = ausis.input()
    else:
        name = s
    ...
    while(os.path.isfile(CUSTOM_PLUGINS_PATH + filename + ".py") or
          ...
          not exit):
        ausis.say("A plugin with the name '" + filename +
                   "' already exists", Fore.RED)
        ausis.say("Please choose another name or type 'exit' " +
                   "for obvious results:", Fore.CYAN)

        new_name = ausis.input()
        if new_name == 'exit':
            exit = True
    ...
    if(not exit):
        ...
        if file_exists(filename):
            ausis.say(filename + ".py created successfully inside " +
                       "ausis/custom", Fore.CYAN)
            string = "open " + CUSTOM_PLUGINS_PATH + filename + ".py"
            os.system(string)
        else:
            ausis.say("Something went wrong in the creation of the plugin :(",
                       Fore.RED)
...
@plugin("create plugin")
def create_plugin_LINUX(ausis, s):
...
def file_exists(filename):
    ...
    the name "filename" exists in the ausis/custom folder
    """
...
# This is the standard form of a plugin for ausis

# Anytime you make a change REMEMBER TO RESTART ausis

# You can run it through ausis by the name
# in the @plugin tag.


@plugin("my plugin")
def my_plugin(ausis, s):
