...
def buy(ausis, s):
    ...
    if len(cut_string) > 1:
        ...
        if len(endURL) < 1:
            ausis.say("Wrong input. Try again or try with just 'buy'.")
            return None

    else:
        ausis.say("Pick a site (Amazon or Ebay)")
        shop_input = ausis.input()
        startingURL = shop(shop_input)
        if len(startingURL) < 1:
            ausis.say("Pick between Amazon or Ebay. Please try again.")
            return None
        ausis.say("What you need to buy?")
        search_term = ausis.input()

        endURL = generateURL(startingURL, search_term, False)
        if len(endURL) < 1:
            ausis.say("Empty search term. Please try again.")
            return None
